...
|-------|----------------|------------|
| Figma write-back tool | `generate_figma_design` writes generated UI back to the canvas | `generate_figma_design` is the code-to-canvas tool (turns rendered UI into flat Figma frames). The real general write-back tool is `use_figma`, and it is beta quality with a 20kB per-call cap, no image or asset support yet, and fonts must pre-exist |
